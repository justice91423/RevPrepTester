...
var Page = function(browser){
	...
	this.find = function(el,by='cssDefault',mouseOver=false,moveTo=false,x=0,y=0){
		...
		// if (moveTo == true && by == 'xpath'){
		// 	// this is used to click things that aren't visible or covered by other elements.  It should really be a separate function.
		// 	this.debug("in moveTo if in find() in base_page");
		// 	driver.actions().moveToElement(foundElement,x,y).click().build().perform()
		// }
	...
	this.setWindowSizePosition = function(){
		driver.manage().window().setPosition(0, 0);
      	driver.manage().window().setSize(2000,1080);
	}
}
    ...
    .then(function findMatchingOption(options){
        ...
        options.some(function(option){
            ...
            option.getText().then(function doesOptionMatch(text){
                ...
                if (item.replace(/\s/g, '') === text.replace(/\s/g, '')){
...
Page.prototype.clickClearWrite = function(text,el,by){
	this.debug("clickClearWrite " + text +" "+el+" "+by)
...
Page.prototype.networkWait = function(interval){
	return sleep(interval * 3)
} 

Page.prototype.countElements = async function(locator){
	this.debug("countElements "+locator)
	return page.driver.findElements(By.xpath(locator))
}
