...
Page.prototype.fillmodifiableCriteriaField = function(name,value){
	...
	.then((text) => {
		this.debug('fillmodifiableCriteriaField text.replace(" $","")='+text.replace("$","")+ "value=" +value)

		if(!(value.includes(text.replace(" $","")))){
			this.debug("fillmodifiableCriteriaField text="+text.replace(" $","")+ "value=" +value)
...
var selectCriteriaFields = ["Advisor","Leads","Products","Contact Attempts","Contact List Reason","Grade","Order Source","Source","Last NPS","School Type"];
