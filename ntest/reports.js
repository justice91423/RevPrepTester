...
function tests(browser){
  describe('Admin Dashboard Report screen scenarios - '+browser, function(){
    ...
    it('Transactions Report screen can be reached', function(done){
      this.retries(trys)

      page.clickReportsOption("transactions")
      .then(() => sleep(500))
      .then(() => page.purchaseMembership())
      .then((gotback) => console.log("gotback",gotback))
      .then(() => page.getInnerHTML(transaction_reports_title_xpath,'xpath'))
      .then((verificationText) => (verificationText.txt.should.eventually.equal("Transactions Report", "Could not reach Transactions Report page").notify(done)))
    });
