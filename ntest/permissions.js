...
function tests(browser){
  describe('Admin Dashboard permission scenarios - '+browser, function(){
    ...
    function navbarItemTest(role){
      ...
      if(role.deleteExam){
        it(role.name +' can delete exams', function(done){
          this.retries(trys)
          if(passing==false){
            this.retries(0)
            role.name.should.equal(true,role. name+' was not set properly.  This test can not be run')
          ...
          .then(() => sleep(500))
          var verificationText = page.getInnerHTML('//button[contains(., "Delete Exam")]','xpath');
          verificationText.txt.should.eventually.contain('Delete Exam').notify(done);
        });
      }
      ...
      if(role.setHandoff){
        it(role.name +' can set Handoff via CRM', function(done){
          ...
          .then((gotten) => assert.lengthOf(gotten, 1, role.name+" can NOT set Handoff via CRM"))
          .then(() => done())
        });
      }
