...
function tests(browser){
  describe('Admin Dashboard Transaction Report scenarios - '+browser, function(){
    ...
    // function createLeadTest(leadSourceType="Gift Card",status="Pre-Conversation"){
      it('transaction report', function(done){
        this.retries(trys)
        ...
        page.enterFiltersTransactionReport()
        ...
        .then(() => {
          ...
          verificationSourceText.txt.should.eventually.equal("sdsad", "Lead had the wrong source").notify(done);
        });
      });
    // };
