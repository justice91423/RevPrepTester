...
function tests(browser){
  describe('Admin Dashboard permission scenarios - '+browser, function(){
    ...
    function navbarItemTest(name,singleItems,sectionsAndItems,optionNumber){
      ...
      it('Set '+name+' as the users Role',  function(done){
        passing = true;
        ...
        .then(() => setBasicAdmin())
        .then(() => page.addARoleEditEmployeeModal(name))
        .then(() => page.clickUpdateEditEmployeeModal())
        .then(() => page.clickEditedEmplyeeButton())
      ...
      it(name+' role provides correct Navbar items', function(done){
        ...
        if (name == "Spoof Advisor"){
          it('Spoof Advisor adds ability to spoof',  function(done){
            this.retries(trys)
            if(passing==false){
              this.retries(0)
            ...
            .then((gotten) => assert.lengthOf(gotten, 1, "The Spoof Advisor option is NOT present"))
            .then(() => done())
          });
        }
